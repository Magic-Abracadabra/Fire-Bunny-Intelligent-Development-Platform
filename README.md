...
# 官方安装包唯一下载地址
<a href="ms-windows-store://pdp?productid=9pllc5bf668t" title="微软应用商店"><img src="https://get.microsoft.com/images/zh-cn%20light.svg" alt="微软应用商店" title="微软应用商店"></a>
...
# 几种常见的自动化办公场景
1. 【用汉语配置自动登录模块】使用本地的密码自动登录一些暂时不支持自动登录的网站——放心把密码留在本地，无需交给AI Agent
2. 【用汉语更轻松地抓取网上的数据】作为爬虫的代替方案，作为用户能力的延伸，可避免爬虫因权限不足（因为RPA操纵的浏览器可以保持登录）而面对的数据割据难题
3. 【用汉语操控第三方软件实现工作的批处理】使用本平台的中文高级计算机语言编写中文命令，利用本平台操控第三方软件，比如用PS批量处理好图片后复制进入PowerPoint并完成PPT编排的自动化
4. 【用汉语定期更新海量数据】使用本平台，进入近100个不同的网站里面定期更新总共约360列的时间序列数据，保存到Excel中，将重复的、工作量大的工作完全自动化——人可以做其它更有意义的事，让机器日夜无休
