...
It's our first step/project of the [Self-Sovereign Office Agents and Their Incremental Learning Framework](https://github.com/Magic-Abracadabra/Self-Sovereign-Office-Agents-and-Their-Incremental-Learning-Framework) series.
...
# 教育用途示例
1. 本平台重视汉语语法，设计基于汉语语法的基本框架（包括标点的使用），有助于培养学生的交叉学科思维
2. 本平台的代码直观简洁，可帮助中国低年级学生培养算法与模块化素养

# 特色声明
1. 本平台的使用**无需联网**，这有别于主流AI Agent，能降低隐私泄露风险——我们极致地尊重隐私
2. 本平台的命令**完全可解释且有确定的、可审计的执行方式**，这有别于主流AI Agent的黑箱、不可控特征

# 平台简介
一款使用中文命令快速搭建自动化办公组件的开源办公平台，代表一种专注于流程自动化场景的中文高级计算机语言

# 几种常见的自动化办公场景
1. 【用汉语配置公司内网的自动登录模块】使用本地的密码自动登录内网一些暂时不支持自动登录的网站——放心把密码留在本地，无需交给AI Agent
2. 【用汉语更轻松地整理网上的数据】作为用户能力的延伸，通过保持登录解决数据割据难题
3. 【用汉语操控第三方软件实现工作的批处理】使用本平台的中文高级计算机语言编写中文命令，利用本平台操控第三方软件，比如用PS批量处理好图片后复制进入PowerPoint并完成PPT编排的自动化
4. 【用汉语定期更新海量数据】使用本平台，进入近100个不同的网站里面定期更新总共约360列的时间序列数据，保存到Excel中，将重复的、工作量大的工作完全自动化
...
1. [调试窗口（演示）](https://github.com/Magic-Abracadabra/Fire-Bunny-Intelligent-Development-Platform/blob/main/Demo.mp4)：直接双击“机关区”，进入界面之后就可以直接编写，调试，编辑，浏览，运行本平台命令；支持一行多个句子的命令，但不支持多行输入
    ...
    * 当输入内容不是本平台代码时按下回车，触发哭脸“【ToT】”

2. [火小兔——执行开发的脚本文件（本项目联网对照组的交互演示）](https://github.com/Magic-Abracadabra/AI-Chinese-Scripting-Language/blob/main/Demo%202.mp4)：“机关文件”与“秘籍文件”是本平台的脚本文件，可用记事本编辑开发；把“机关文件”与“秘籍文件”分别用“机关区”与“火小兔”打开；前者运行期间，窗口始终存在，后者则在初始化阶段有窗口，初始化结束后，运行窗口将被隐藏
